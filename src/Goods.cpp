...
//        srand((unsigned int)time(NULL));
    ...
    {
        ofstream ofs("./source/goods.txt",ios::out|ios::app);
        ofs<<left<<setw(10)<<num<<setw(15)<<left<<name<<setw(10)<<left<<price<<endl;
        ofs.close();
    }
    void goods::Save2()         //goods 的保存
    {
        ofstream ofs("./source/goods.txt",ios::out|ios::trunc);
        ofs<<left<<setw(10)<<num<<setw(15)<<left<<name<<setw(10)<<left<<price<<endl;
        ofs.close();
    }
    //读取文件
...
//}
    ...
    bool goods::cutout(goods * Goods ,int n,int number)
    {
        this->Read(Goods ,n);
        int flag = 1;
            ofstream ofs("./source/goods.txt",ios::trunc);
            ofs.close();
            for(int i=0;i<n;i++)
            {
            ...
            if(Goods[i].num == number)
            {
                ...
                ofstream ofs1("./source/./source/goodsinfo.txt",ios::out|ios::app);
